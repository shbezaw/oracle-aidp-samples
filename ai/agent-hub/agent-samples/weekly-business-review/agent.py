...
from oci_llm import build_llm, pre_invoke_setup
...
A2UI_VERSION = "0.8"
DEFAULT_CATALOG_ID = "/a2ui_specification/1.0.0/agent_hub_a2ui_custom_component_catalog.json"
ROOT_ID = "response_root"
ALLOWED_TEMPLATES = {
...
def build_tools() -> list[Any]:
    ...
    return [
        ...
        _sql_tool(
            ...
            "SELECT account_id, account_name, tickets_opened, p1_p2_tickets, sla_breaches, "
            "avg_response_hours, open_escalations FROM support_health_by_account "
            "WHERE week_start >= TO_DATE({{start_date}}, 'YYYY-MM-DD') "
            "AND week_end <= TO_DATE({{end_date}}, 'YYYY-MM-DD') "
            "AND region = {{region}} AND segment = {{segment}} "
            "ORDER BY open_escalations DESC, sla_breaches DESC FETCH FIRST 12 ROWS ONLY",
            params,
        ),
    ]
...
def _component(component_id: str, name: str, props: dict[str, Any], weight: int | float | None = None) -> dict[str, Any]:
    item: dict[str, Any] = {"id": component_id, "component": {name: props}}
    if weight is not None:
        item["weight"] = weight
    return item


def _text(component_id: str, value: str, usage: str = "body") -> dict[str, Any]:
    return _component(component_id, "Text", {"text": _literal(value), "usageHint": usage})


def _column(component_id: str, children: list[str], weight: int | float | None = None) -> dict[str, Any]:
    return _component(component_id, "Column", {"children": {"explicitList": children}, "distribution": "start", "alignment": "stretch"}, weight)


def _row(component_id: str, children: list[str], distribution: str = "spaceBetween") -> dict[str, Any]:
    return _component(component_id, "Row", {"children": {"explicitList": children}, "distribution": distribution, "alignment": "stretch"})
...
def _button(component_id: str, label_id: str, label: str, name: str, context: list[dict[str, Any]], primary: bool = False) -> list[dict[str, Any]]:
    return [
        _component(component_id, "Button", {"child": label_id, "primary": primary, "action": {"name": name, "context": context}}),
        _text(label_id, label),
    ]


def _filter_context(filters: dict[str, str], template_id: str) -> list[dict[str, Any]]:
    return [
        {"key": "template_id", "value": _literal(template_id)},
        {"key": "start_date", "value": {"path": "/wbrFilters/start_date"}},
        {"key": "end_date", "value": {"path": "/wbrFilters/end_date"}},
        {"key": "region", "value": {"path": "/wbrFilters/region"}},
        {"key": "segment", "value": {"path": "/wbrFilters/segment"}},
        {"key": "current_scope", "value": _literal(json.dumps(filters, sort_keys=True))},
    ]
...
def render_operations(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for section_name, title in (("insights", "Insights"), ("recommendations", "Recommendations")):
        ...
        row_id = f"{section_name}_row"
        card_ids = []
        root_children.extend([heading_id, row_id])
        components.append(_text(heading_id, title, "h3"))
        for index, section in enumerate(sections, 1):
            card_id = f"{section_name}_{index}_card"
            column_id = f"{section_name}_{index}_column"
            card_ids.append(card_id)
            components.extend([
                _card(card_id, column_id, 1),
                _column(column_id, [f"{section_name}_{index}_title", f"{section_name}_{index}_body"]),
                _text(f"{section_name}_{index}_title", section["heading"], "h3"),
                _text(f"{section_name}_{index}_body", section["body"], "body"),
            ])
        components.append(_row(row_id, card_ids))
    ...
    if actions:
        ...
        for index, action in enumerate(actions, 1):
            ...
            components.extend(_button(
                ...
                action["label"],
                "ask_wbr_question",
                [{"key": "prompt", "value": _literal(action["prompt"])}],
            ))
    ...
    operations = [
        {
            "dataModelUpdate": {
                ...
                "path": "/wbrFilters",
                "contents": [
                    {"key": "start_date", "valueString": filters["start_date"]},
                    {"key": "end_date", "valueString": filters["end_date"]},
                    {"key": "region", "valueString": filters["region"]},
                    {"key": "segment", "valueString": filters["segment"]},
                ],
            }
        },
        {"surfaceUpdate": {"surfaceId": surface_id, "components": components}},
        {"beginRendering": {"surfaceId": surface_id, "catalogId": _catalog_id(), "root": ROOT_ID}},
    ]
...
def validate_operations(operations: list[dict[str, Any]]) -> None:
    if [next(iter(item), "") for item in operations] != ["dataModelUpdate", "surfaceUpdate", "beginRendering"]:
        raise ValueError("A2UI operations must be dataModelUpdate, surfaceUpdate, then beginRendering.")
    components = operations[1]["surfaceUpdate"].get("components")
    if not isinstance(components, list) or not components or components[0].get("id") != ROOT_ID:
        raise ValueError("surfaceUpdate requires a root-first component list.")
    ids = [item.get("id") for item in components]
    if len(ids) != len(set(ids)):
        raise ValueError("A2UI component IDs must be unique.")
    referenced: set[str] = set()
    for item in components:
        wrapper = item.get("component")
        if not isinstance(wrapper, dict) or len(wrapper) != 1:
            raise ValueError("Each A2UI component must wrap one component type.")
        name, props = next(iter(wrapper.items()))
        ...
        if isinstance(props, dict) and isinstance(props.get("child"), str):
            referenced.add(props["child"])
        if name == "OAChart":
            for chart_item in props.get("items", []):
                missing = {"id", "seriesId", "groupId"} - set(chart_item)
                if missing:
                    raise ValueError(f"OAChart item missing hosted metadata: {sorted(missing)}")
        if name == "OADataGrid":
            if any("id" not in row for row in props.get("rows", [])):
...
def normalize_request(user_query: Any, kwargs: dict[str, Any]) -> str:
    ...
    if isinstance(metadata, dict):
        candidates.extend([metadata.get("userAction"), metadata.get("action")])
    for candidate in candidates:
        ...
        if isinstance(context, list):
            ...
            for item in context:
                ...
                if isinstance(value, dict):
                    value = value.get("literalString", value.get("literal", value.get("path")))
                decoded[item["key"]] = value
            context = decoded
        if action["name"] == "ask_wbr_question" and isinstance(context, dict) and context.get("prompt"):
        ...
        return "The user applied this Weekly Business Review action. Run the required SQL tools with the resolved filter values and return an updated analysis: " + json.dumps(
            {"name": action["name"], "context": context}, ensure_ascii=True
        )
